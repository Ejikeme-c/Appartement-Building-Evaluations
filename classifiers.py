...
class NearestNeighbour:
    ...
    def predict(self, apartment: ApartmentBuilding) -> float:
        ...
        91
        """
        target = apartment.apartment_cosmetic_rating()

        nearest = self._training_data[0]
        nearest_dist = abs(nearest.apartment_cosmetic_rating() - target)

        for r in self._training_data[1:]:
            dist = abs(r.apartment_cosmetic_rating() - target)
            if dist < nearest_dist:
                nearest = r
                nearest_dist = dist

        return nearest.apartment_overall_rating()
    ...
    def make_predictions(self, apartments: list[ApartmentBuilding]) -> dict[ApartmentBuilding, float]:
        ...
        for apartment in apartments:
            prediction_r = self.predict(apartment)
            predictions[apartment] = prediction_r
...
class LinearRegression:
    ...
    def train(self, apartments: list[ApartmentBuilding]) -> None:
        ...
        4.6
        """
        self._xs = [a.apartment_cosmetic_rating() for a in apartments]
        self._ys = [a.apartment_overall_rating() for a in apartments]
    ...
    def predict(self, apartment: ApartmentBuilding) -> float:
        ...
        x = apartment.apartment_cosmetic_rating()
    ...
    def make_predictions(self, apartments: list[ApartmentBuilding]) -> dict[ApartmentBuilding, float]:
        ...
        """
        if not self._xs and not self._ys and self._a == 0 and self._b == 0:
            raise ValueError('Model must be trained before making predictions')

        predictions = {}

        for apartment in apartments:
            predictions[apartment] = self.predict(apartment)
...
def calculate_mse(predictions: dict[ApartmentBuilding, float]) -> float:
    ...
    for apartment, predicted in predictions.items():
        true_rating = apartment.apartment_cosmetic_rating()
        error = true_rating - predicted
        sse += error ** 2

    return sse / n if n > 0 else 0.0
