...
class ApartmentBuilding:
    ...
    def get_all_ratings(self) -> list[float]:
        ...
        [6, 11, 86]
        >>> ApartmentBuilding('SOCIAL HOUSING', 1, 1973, [Review('APT_LOG', 6), Review('APT_SAFETY', 11), Review('APT_COS', 12), Review('APT_OVERALL', 86)]).get_all_ratings()
        [6, 11, 12, 86]
        """
    ...
    def num_ratings(self) -> int:
        ...
        3
        >>> ApartmentBuilding('SOCIAL HOUSING', 1, 1973, [Review('APT_LOG', 6), Review('APT_SAFETY', 11), Review('APT_COS', 12), Review('APT_OVERALL', 86)]).num_ratings()
        4
        """
        count = 0
    ...
    def apartment_min_rating(self) -> tuple[str, float]:
        ...
        ('MODERATE RISK', 6)
        >>> ApartmentBuilding('SOCIAL HOUSING', 1, 1973, [Review('MODERATE RISK', 12), Review('COSMETIC', 11), Review('OVERALL', 86)]).apartment_min_rating()
        ('COSMETIC', 11)
        """
        min_rate = self.reviews[0].rating
        min_rate_type = self.reviews[0].type
        for r in self.reviews:
            if r.rating < min_rate:
                min_rate = r.rating
                min_rate_type = r.type

        return min_rate_type, min_rate
